...
def get_locations(key):
    ...
    return locations_df.to_csv('US_Locations.csv', index=False)


def get_sensorID(sampled_locations, key):
    BASE_URL = "htps://api.openaq.org/v3/locations"

    df = pd.read_csv(sampled_locations)
    ...
    return df.to_csv("Locations_with_SensorIDs.csv", index=False)
               
def sample_location(locations, key):
    ...
    return sampled_df.to_csv("US_Sampled_Locations.csv", index=False)
...
def state_info():
    files = {
        "edu": "MappingAmerica_Education.xlsx",
        "hdi": "MappingAmerica_HDI.xlsx",
        "housing": "MappingAmerica_Housing.xlsx",
        "env": "MappingAmerica_Environment.xlsx",
    }
...
def fetch_averages(input_csv, output_csv, key):
    headers = {"x-api-key": key}
...
def merge_data(sensor_avg, state_info):
	...
	sensor_df = pd.read_csv(sensor_avg)
	state_df = pd.read_csv(state_info)

	mean = sensor_df.groupby("State")['PM25_Yearly_Avg'].mean()
